...
이 각도로 가면 플랫폼이 왜 현대적 주권자인지 선명하게 나온다.
...
"생존조건을 쥠"이 사용자 유형에 따라 연속값이라는 것이 여기서 선명해진다.
...
| 데이터 이동성 | 매우 낮음 (제도적 이동성 부재) | 매우 낮음 (소셜 그래프 이동 불가) |
| 국제 비교 | 다른 국가의 비자 제도 존재 | 글로벌 플랫폼은 대체재 부재 |

### 공통점: 강제의 역설

두 사례 모두 처방을 채택할 권한을 가진 주체가 처방에 의해 자신의
...
프레임워크가 요구하는 다섯 가지 — 설명, 이의제기, 집행정지, 배상,
...
| 자동처분 로그 공개 | 알고리즘 정지의 판정 기준·임계치·오패율 공개 | 설명 + 측정권 공개 |
...
- **국가 규제**: 국가가 다시 주권자. 다만 이 경우 국가는 "통치 강제"가
  아니라 "탈출권 강제"를 수행하는 것이므로, 프레임워크의 방향에 부합.
...
```
플랫폼 유형        | 이탈가능성 | 규칙강제성 | 항의실효성 | 의존비대칭 | 위험도
------------------|----------|----------|----------|----------|------
...
2. **교차점에서 창발하는 락인은 어느 쪽 책임도 아님** — 단일 부문
   감사로는 보이지 않음.
...
- **가족 돌봄 의존 (돌봄 락인)** — "탈출 불가능자"가 가장 선명하게
