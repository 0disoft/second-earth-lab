...
### 명제 2: 문제는 권력이 아니라 락인(갇힘)이다

현대 주권은 영토보다 **락인(lock-in)**에서 생긴다. 주권의 원천을 지리가
아니라 구조적 락인에서 찾아야 한다. 이 판별식(갇힘 확인표)의 파괴력은 적용 범위에 있다 —
...
| **집행정지(멈추기)** | 치명적 피해가 예상되면 집행을 멈출 수 있음 |
| **배상** | 틀렸으면 배상 |
...
합작 락인(짝꿍 갇힘)의 측정은 교차항 측정이 필요하다: "국가의 체류 규칙과 고용주의
계약 구조가 결합했을 때 탈출 비용이 Y가 된다." 단일 부문 감사로는 보이지
않는다.

처방(해결책)도 단일 주체를 겨냥하면 안 된다. 한쪽만 풀리면 락인이 반쯤 풀리며,
...
1. **각 주체는 자기 역할만 수행** — 국가는 "체류질서", 고용주는 "근로계약".
   ...
   준수", 입시기관은 "선발 기준".
2. **교차점에서 창발(교차점에서 생김)하는 락인은 어느 쪽 책임도 아님** — 단일 부문 감사로는
   보이지 않는다.
...
주권 유형 — 지역, 비지역, 물리적, 관계적, 인정 체계(인정 시장) — 모두에서 동일하게
발생한다.
...
| 의료 + 정부 인허가(허가) | 병원 (의료 서비스) | 정부 (설립·축소 승인) | 지역 의료 접근 단결 |
