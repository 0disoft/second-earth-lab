...
> 반주권 설계 v4 프레임워크에 새 사례를 적용할 때 사용하는 템플릿.
...
2. **교차점에서 창발하는 락인** — [단일 부문 감사로 보이지 않는 것]
3. **처방은 교차점을 동시에 풀어야** — [한쪽만 풀면 어떻게 되는지]
...
```
[유형 A]     | 이탈가능성 | 규칙강제성 | 항의실효성 | 의존비대칭 | 위험도
[유형 B]     |           |           |           |           |
[유형 C]     |           |           |           |           |
```
