...
> 반주권 설계 v4 프레임워크의 다섯 번째이자 마지막 적용 사례.
...
이 사례가 프레임워크를 세게 시험하는 이유: 데이터 이동성 3층 중 **3층(제도적
...
프레임워크의 "대체 생존조건 동반 원칙"이 교육에도 적용된다: 학교를
...
| 학생 대리 발언권 | 독립 옴부즈 | 학생 권리 옴부즈맨, 정기 접견, 징계 심사 참여 | 대리 발언권 |
| 사교육 의존도 공개 | 커널(측정) | 학군별 사교육비·시간 공개, 입시 성과 상관관치 공개 | 측정권 공개 |
...
```
교육 트랙              | 이탈가능성 | 규칙강제성 | 항의실효성 | 의존비대칭 | 위험도
----------------------|----------|----------|----------|----------|------
...
학력 인정이 부재하여 의존 비대칭이 매우 높다. "나갈 수는 있지만 나가면
...
3. **락인 위험도의 연속성** — 같은 구조에서 사용자/환자/돌봄/학생 유형에
   따라 위험도가 다름
4. **데이터 이동성 3층이 모든 영역에서 진짜 병목** — 1층만 충족되고
